...
```
Double check this entire interpretation:
Doubt: There is significant evidence of clustering at lower elevations  (till about 25000 metres as seen from the plot). ??
...
```{r}
# printing the number of NA values in each covariate
sum(is.na(DATA$Elevation$v))
sum(is.na(DATA$Forest$v))
sum(is.na(DATA$HFI$v))
sum(is.na(DATA$Dist_Water$v))

cor.im(DATA$Elevation$v, use="pairwise.complete.obs")
```

As most of the covariate data has NAs, we cannot check the collinearity among the covariates.
...
forest = DATA$Forest
hfi = DATA$HFI
distwater = DATA$Dist_Water

fit <- ppm(apis_data_clean ~ elevation + I(elevation^2) + forest + I(forest^2), data = apis_data_clean)
```
